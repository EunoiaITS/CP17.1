                ...
                <div class="col-sm-12">
                    <table class="table table-bordered">
                        ...
                        <?php foreach($iPI as $iPI): ?>
                            <tr>
                                <td><?= h($iPI->id) ?></td>
                                <td><a href="#"><?= h($iPI->sn_no) ?></a></td>
                                <td><?php echo date('Y-m-d', strtotime($iPI->created)); ?></td>
                                <td><?= h($iPI->manufacturer) ?></td>
                                <td><?= h($iPI->checked_by) ?></td>
                                <td><?= h($iPI->remarks) ?></td>
                                <td>
                                    <ul>
                                        <li><a href="<?php echo $this->Url->build(['controller' => 'IPI', 'action' => 'confirm', $iPI->id]); ?>"><img class="img-icon" src="<?php echo $this->request->webroot.'assets/img/search.png'; ?>" alt=""></a></li>
